...
木目（我们）非常重视对您的个人信息的保护。木目隐私政策(“本政策”)旨在向您说明，在您下载、安装、开启、浏览、使用及注销（以下统称“使用”）木目（以下称“产品”或 “服务”）时，我们承诺对您个人信息处理的最大限度不超过《中华人民共和国个人信息保护法》、国家的相关法律法规及本政策的范畴。。
...
在您注册木目帐号、使用本产品和服务时，您需要提供可用于实名认证的手机号或社交媒体账号。

**2、用于实名认证**

当您在申请提现、达人身份认证、借钱等其他需要实名认证的功能或服务时，为满足法律法规的合规需求或合同签署之目的，保障您的财产安全等，需要您提供**真实身份信息（如真实姓名、身份证号码等）**以完成实名认证，上述信息为**敏感个人信息**，收集此类信息前会征求您的单独同意。我们会对此进行严格保密，并不会将上述信息用于本政策以外其他目的。同时，您也可拒绝提供上述信息，这将导致您无法使用需要实名认证的特定功能，但不影响您继续使用木目其他功能或服务。在完成实名认证过程中，为保证信息真实性，我们可能委托提供认证服务的第三方认证机构核验您提供的认证信息。

**3、向您提供网络安全及运营安全保障**

为保障您的帐号、网络、运营及系统安全，预防钓鱼网站欺诈及木马病毒，在您使用本产品的过程中，我们可能会收集您的设备信息，包括不可变更的设备标识信息（指由设备制造商编入到设备以对其进行标识的一串字符，**Android如IMEI、AndroidID、OAID、IMSI、ICCID、GAID、MEID、AAID、VAID，iOS如IDFV、IDFA，本政策中所指的“设备标识信息”均同本定义）、硬件序列号、MAC地址。**请您理解，当您将处于设备屏幕中的木目APP切换至后台运行时，由于部分设备机型及系统原因，您此前使用木目相关功能而产生的信息收集行为，可能无法立即停止，从而将导致短暂的后台信息收集行为。

**4、向您提供其他相关产品功能和/或服务**

实现以下各项产品功能和/或服务，我们可能需要向您收集相对应的个人信息，我们将遵循合法、正当、必要的原则谨慎收集；若您拒绝提供，可能无法使用相应服务。其中个别功能或服务可能涉及人脸图片等敏感个人信息（详见以下具体相关条款），在您使用相关功能或服务前我们还将征求您的单独同意；如您不同意仅无法使用对应功能或服务，其它功能或服务仍可正常使用。

- 1）客户服务。当您向木目提起投诉、进行咨询或申诉时，为了方便与您联系或帮助您解决问题，我们可能需要您提供**姓名、手机号码、电子邮件**及**其他有效的联系方式**等个人信息。 如您拒绝提供上述信息，我们可能无法向您及时反馈相关处理结果。
- 2）当您使用木目中提供的特效、滤镜、贴纸等工具拍摄并发布内容时，我们会根据您的操作及使用的工具类型使用传感器信息，以适配您所选用的工具。当您拍照或浏览内容时，为了适配您的设备状态，我们会调用您设备的旋转矢量传感器、加速度传感器信息，以识别您的设备横竖屏状态，我们不会保存该场景下的传感器信息。
- 3）个性化展示服务。为了便于您更好地管理您的偏好，获得更轻松的访问体验，我们会基于收集的信息，对用户偏好进行特征分析和用户人群画像（该个性化推荐算法不会识别特定自然人的真实身份），以帮助您在产品社区中获得您更感兴趣的社区内容，或在工具素材和广告内容推荐上呈现更符合您需求的内容，以减少您对海量互联网内容的筛选时间。为此，我们需要收集的信息包括您的**设备标识信息、符合国家或行业团体标准的广告标识信息、地理位置信息（如GPS）、语言及时区、您的应用列表信息及木目中的浏览记录、正在运行的进程信息，**其中**精确的地理位置信息**需要您的授权，若您拒绝授权位置权限会影响定位的准确性，我们将无法向您推送最为精确的个性化内容和广告，但不会影响其他功能与服务的正常使用，我们也仅会基于**网络信息（如IP）**对应的城市及/或区县向你展示可能相关的内容、服务和广告。如果您对我们的个性化展示内容或广告不感兴趣，您可以通过本协议明示的方式关闭该功能。
- 4）“摇一摇”等依赖于动作开展的互动 当你进行“摇一摇”、“抖一抖”等依赖于动作开展的互动或活动时，我们会使用你的传感器信息，以识别你的动作，用于活动参与或为你展示下一步互动场景，**我们不会保存该场景下的传感器信息**。
- 5）我们还可能收集您的**网络运营商及种类、网络类型、WIFI名称·（SSID BSSID），**以实现在广告推送过程中帮助您节省网络流量的目的，例如：当我们监测到您处于WIFI环境下，我们可能为您直接播放视频类广告内容；当我们检测您处于无网络、或弱移动网络的环境下，我们将仅为您推送您已经缓存于本地的广告内容。
- 6）维护产品安全及时排除故障。为维护我们服务的正常运行，以及保障您的帐号安全、排查产品故障、改进及优化我们的服务，我们会收集您的**EXIF数据、设备标识信息、设备型号信息及登陆IP地址、操作日志信息、设备品牌、系统版本、CPU信息。**这类信息是为提供服务必须收集的信息，收集后即作去标识化处理，不会用于识别任何特定的用户。为了保证人脸图像处理效果，我们的服务可能会利用苹果的**ARKit和TrueDepth应用程序**接口中人脸映射深度相关信息的计算结果，在此期间，我们既不会将该等计算结果用于其他目的，也不会为记录之目的或以其他方式收集您的面部映射深度的目的而捕获文件中的任何数据或向服务器上传数据。
- 7）保障帐号安全。当您在使用修改密码、安全验证、申诉行为，且操作行为存在异常时，我们可能会要求您提供**有效身份信息**，以确保上述操作行为的安全性。
- 8）当您使用图片美化、人像美容功能时，我们会收集不识别任何特定个人的粗略间接画像特征（如大类别：风景、美食、人像等）并通过算法进行图片特征分析，用以帮助您选择更加精准的滤镜、特效等。
- 9）用于调起第三方支付等功能。。
- 如您使用木目中支付功能时，我们会读取您的**应用列表信息**用以判断该第三方支付应用是否存在以完成支付功能。
- 10）用于内容分享
- 如您将木目中内容分享至第三方平台时，我们会读取您的**应用列表信息**用以判断该第三方平台是否存在从而达成分享目的；如您在使用口令功能进行分享、兑换邀请码时，我们需要在本地访问您的**剪切板信息**，读取其中包含的口令，以实现内容分享。我们仅在本地识别出剪切板信息属于口令时，才会将其上传服务器，除此之外，我们将不会上传剪切板的其他信息至服务器。
- 11）美颜记忆服务。当您使用并调整木目的美颜参数时，例如调整了磨皮、大眼等参数，我们会记录您的调整，以便下次启动时直接给您匹配适合的参数，方便您的使用。
- 12）AI算法优化服务。当您使用人脸生成服务时人像美容或美化的部分滤镜和贴纸修图、以及用相机拍照时，我们会采用木目人脸点数据技术收集您的**皮肤和五官参数**。这是为了实现AI算法优化，更准确快速地反馈检测报告。使用次数越多，AI算法优化效果越好。这类信息是为提供本服务必须收集的信息，我们会按照法律的要求，对您的该类信息在收集、传输和存储的过程中采取严格的加密措施，以确保您信息的安全。我们仅提供图像处理服务，不会提取识别信息，收集后即作去标识化处理，不会用于识别任何特定的用户，也不会分享给第三方。。
- 13）iOS人脸优化处理效果。当您使用萌拍时，我们可能会利用iOS的 **ARKit 和 TrueDepth** 应用程序接口中人脸映像深度相关资讯运算结果，以达到更好的人脸影像处理效果；在此期间，我们既不会将该等运算结果用于其他目的，也不会以记录为目的，或以其他方式搜集您的脸部映像深度为目的，撷取档案中的任何资料或上传资料到服务器。
- 14）图像分类服务。当您使用木目配方、智能优化等功能时，我们会收集不识别任何特定个人的**粗略间接画像特征**（如大类别：风景、美食、人像等），并通过算法进行图片特征分析，用以帮助您选择更加精准的配方等。
- 15）身体轮廓识别服务。当您使用智能抠图、马赛克人像保护、小头等功能时，我们会用实例分割技术识别您的**头部和身体轮廓参数**。这类信息是为提供本服务必须收集的信息，收集后即作去标识化处理，不会用于识别任何特定的用户。
- 16）增发功能。当您使用增发功能时，**您需上传人脸图像信息进行图像处理，生成发际线、刘海等效果。**生成的图像将返回您的设备终端。该信息的上传和处理为提供本功能所必须。我们会按照法律的要求，对您的该类信息在传输和存储的过程中采取严格的加密措施，以确保您的信息安全，对该信息的使用仅限于为您提供本功能服务。服务完成后，系统将自动删除上述信息，不予留存。
...
B、旧手机换钱服务由北京友传网络科技有限公司提供，我们会向其共享您的用户uid， 用于快速登陆。
...
1、我们的产品主要面向成人。除非您在使用某些特定功能（如需要实名认证的提现、借钱服务等）时，我司可通过实名认证信息判断您为未成年人外，受限于本产品特性，我司并未开发其他识别您真实年龄的产品功能。

2、不满十四周岁未成年人（本政策所指“未成年人”，特指不满十四周岁的未成年人），在未获得父母或监护人同意的前提下，不应创建自己的个人信息主体帐户。如果您的年龄在14周岁以下，在使用我们的产品和/或服务前，请您务必在监护人的陪同下阅读本政策，并确保已征得您的监护人明确同意后，再使用我们的服务。
...
6、如果我们发现在未获得监护人同意的情况下收集了未成年的个人信息，则会设法尽快删除相关未成年人信息，同时，为保护未成年人个人信息安全，我们设置了专门的[《未成年人个人信息保护规则》](https://pro.meitu.com/xiuxiu/agreements/juvenile.html?lang=zh) ，请您仔细阅读。
...
本木目个人信息保护政策适用中华人民共和国大陆地区法律。本木目个人信息保护政策项下争议，由**被告住所地法院管辖**。

**九、本政策如何更新**

本我们的木目个人信息保护政策可能会根据业务的调整、法律法规或政策的变化而适时变更。未经您明确同意，我们不会削减您按照本木目个人信息保护政策所应享有的权利。本政策更新后，我们会发出更新版本，并在更新后的条款生效前通过私信、通知、公告或其他适当的方式提醒您更新的内容，以便您及时了解本政策的最新版本。

如本政策变更的类型涉及个人信息的处理目的、处理方式和处理的个人信息种类发生变更的，我司将重新取得您的同意。

若您不同意修改后的木目个人信息保护本政策，您有权并应立即停止使用我们的产品和/或服务。如果对本政策的更新存在疑义，请您及时通过本政策公示的联系方式与我方联系。

**十、我们的联系方式**

我公司为在中华人民共和国境内成立的【厦门鸿天创视科技有限公司】，注册地址：厦门火炬高新区软件园华讯楼C 区B1F-055。

如果您对本政策内容有任何疑问、意见、建议或进行申诉，您可通过以下方式联系我们：请发送电子邮件至个人信息保护专员邮箱：<legal@meitu.com>，也可以挂号信方式寄送至：中华人民共和国厦门市同安区美峰大厦2号楼（收件人：木目公司-法务部）。
...
| :- | :- | :- |
|网络访问权限（android）|访问网络信息|用于下载和运行游戏。|
|电话权限|读取手机状态|读取手机状态，涉及到用户设备标识信息，用于识别用户、保障您的帐号、网络、运营及系统安全，预防钓鱼网站欺诈及木马病毒及进行用户数据统计（安装、活跃度等）。|
|存储权限（写入）|写入外部存储|用于将用户拍摄完毕的图片、视频保存到本地等的文件保存功能，帮助用户进行图片、视频美化、游戏包正常下载和存储。|
|地理位置权限|接收卫星的定位信息|获取精确位置地理位置的权限。用于用户带有地理位置的水印、素材的获取、第三方广告投放。在用户主动同意打开地理位置权限的前提下，为满足木目判断用户所在位置，达到精准提供上述服务的目的，木目将会每间隔一分钟收集一次您的地理位置以避免因您位置变动而导致的误判。如您不希望地理位置被收集，请您主动关闭该权限。|
|录音权限|录制声音|用于木目的视频录制功能：包括相机模式、视频美化中的拍视频功能。|
|相机权限|允许使用相机|用于实名认证、产品的拍照或者视频录制的功能：包括相机、图片美化、人像美容、拼图、视频美化、木目AI、木目数字收藏；|
|蓝牙服务|允许使用蓝牙服务|使用蓝牙权限。用于自拍杆功能|
|存储权限（读取）|读取外部存储|读取本地图片、视频、游戏包等文件。|
